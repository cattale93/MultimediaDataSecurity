function [b, eH, HorzMid, eV, VertMid, BlockDiff] = get_map_BRC(imPath)
    ...
    OutputMap=im2bw(OutputMap,graythresh(OutputMap));
    
    OutputMap=imresize(double(OutputMap),[1500,2000]);
    OutputMap(OutputMap>0.1)=1;
    OutputMap(OutputMap<0.1)=0;
    
    OutputMap = medfilt2(OutputMap,[3 3]);     
    OutputMap = imfill(OutputMap,'holes');
    OutputMap = bwareaopen(OutputMap,12000);
